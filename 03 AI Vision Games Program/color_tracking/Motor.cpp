...
/* TImer1定时器PWM输出(Timer1 PWM output) */
static void Motors_Set(int16_t Motor_0, int16_t Motor_1, int16_t Motor_2, int16_t Motor_3)
{
  ...
  /* 前进 左边轮子需置1 右边轮子需置0(Forward: left wheel needs to be set to 1, right wheel needs to be set to 0) */
...
static void PWM_Out(uint8_t PWM_Pin, int8_t DutyCycle) { //定义一个脉冲函数(Define a pulse function)
...
/* 软件模拟PWM输出(Software simulate PWM output) */
