 ...
 *
 * @attention main函数中不可以做任何阻塞处理！(In main function, do not handle any blocking)
 */
...
typedef enum {   
  ...
  MODE_ULTRASOUND_SEND,   /* 发送超声波距离给上位机(Send ultrasonic distance to the PC) */
  MODE_SERVO_CONTROL,     /* 机械爪控制(Robotic gripper control) */
  MODE_VOLTAGE_SEND,      /* 发送电压值给APP(Send voltage value to the app) */
...
Servo myservo;          /* 实例化舵机(Instantiate the servo) */
Ultrasound ultrasound;  /* 实例化超声波(Instantiate the ultrasonic sensor) */
...
static uint8_t beep_count = 0;      /* 蜂鸣器鸣响次数(Number of beeps from the buzzer) */

static int car_derection = 0;       /* 设置小车移动的角度(Set the angle of the car's movement) */
...
static uint8_t speed_update = 50;   /* APP更新的线速度(Linear velocity updated by app) */

/* 电压监测相关参数(Voltage monitoring related parameters) */
...
static CRGB rgbs[1];
String rec_data[4];                 /* 接收APP的发送数据(Data received from the app) */
...
const static int pwmFrequency = 500;                /* PWM频率，单位是赫兹(PWM frequency in Hz) */
const static int period = 10000000 / pwmFrequency;  /* PWM周期，单位是微秒(PWM period in microseconds) */
const static uint32_t interval_us = 20000;          /* 微秒计数时间间隔 用于非阻塞延时(Microsecond counting time interval used for non-blocking delay) */
const static uint32_t interval_ms = 1000;           /* 毫秒计数时间间隔 用于非阻塞延时(Millisecond counting time interval used for non-blocking delay) */

static uint32_t previousTime_us = 0;          /* 上一次的微秒计数时间间隔 用于非阻塞延时(Previous microsecond counting time interval used for non-blocking delay) */
static uint32_t previousTime_ms = 0;          /* 上一次的毫秒计数时间间隔 用于非阻塞延时(Previous millisecond counting time interval used for non-blocking delay) */ 

static int servo_angle = 0;                   /* 设置舵机角度(Set the angle of the servo) */
static uint16_t distance = 0;                 /* 超声波距离(Ultrasonic distance) */
...
void setup() {
  ...
  myservo.attach(servoPin);                   /* 绑定舵机指定引脚(Bind the servo to the specified pin) */
  myservo.write(servo_angle);                 /* 写入舵机角度(Write the servo angle) */
  tone(buzzerPin, 1200);                      /* 输出音调信号的函数,频率为1200(Output a tone signal with a frequency of 1200) */ 
  delay(100);
  noTone(buzzerPin);
  voltage_send = analogRead(A3)*0.02989*1000;   /* 电压计算(Calculate the voltage) */ 
 ...
 * @arg 三个入口参数取值分别为:0~255;(The three input parameters take values from 0 to 255)
 * @retval None
...
void Task_Dispatcher(void) 
{
  ...
  while (Serial.available() > 0) 
  {
    ...
    while (cmd.indexOf('|') != -1) 
    {
      rec_data[index] = cmd.substring(0, cmd.indexOf('|'));  /* 提取从开始到第一个逗号之前的子字符串(Extract the substring from the beginning to the first comma) */
      cmd = cmd.substring(cmd.indexOf('|') + 1);             /* 更新字符串，去掉已提取的子字符串和逗号(update the string by removing the extracted substring and the comma) */
      index++;      /* 更新索引(Update the index) */
    }
    charArray = rec_data[0].c_str();      /* 转成C字符串形式(Convert to C string form) */
    if(strcmp(charArray, "A") == 0 && avoid_flag == 0)  /* 命令判断(Command judgment) */ 
    {
  ...
  {
    distance = ultrasound.Filter();   /* 获得滤波器输出值(Get the output value of the filter) */
    if(distance > 5000)
    {
      distance = 5000;
    }
...
void Voltage_Detection(void)
{
  ...
  voltage = analogRead(A3)*0.02989;   /* 电压计算(Voltage calculation) */
  ...
  if(g_warning == WARNING_BEEP)
  {
    ...
    {
      tone(buzzerPin, 800);  /* 电压小于7V蜂鸣器警报(Buzzer alarm when the voltage is less than 7V) */
    }
...
void Aovid(void)
{
 ...
 /* 速度调节调节函数(Speed adjustment function) */
void Speed_Task(void) 
{
